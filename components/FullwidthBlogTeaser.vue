<template>
  <FullContainer v-if="featuredArticle">
    <div
    ...
    >
      ...
      >
        <div
          v-if="featuredArticle.category?.name"
        ...
        <div
          v-if="author"
          class="flex justify-start items-center overflow-hidden gap-4 py-4"
        >
          <div
            v-if="author.avatar?.src"
            class="flex justify-start items-start rounded-full"
          >
            <div
      ...
      <div
        class="flex-grow w-full md:w-1/2 relative overflow-hidden rounded-lg"
      >
        <img
          :style="`background-color: ${imageDominantColor}`"
          loading="eager"
          :src="`${imageSrc}?w=960&h=960`"
          :srcset="`${imageSrc}?w=1920&h=1920
        1920w, ${imageSrc}?w=960&h=960 1280w, ${imageSrc}?w=640&h=640 640w,
        ${imageSrc}?w=320&h=320 320w`"
...
const imageSrc = featuredArticle?.teaserImage?.src;
const imageDominantColor = featuredArticle?.teaserImage?.dominantColor;
const imageDescription = featuredArticle?.teaserImage?.description;
